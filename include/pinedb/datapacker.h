...
namespace datapacker
{
    ...
    namespace bytes
    {
        ...
        /**
         * @brief Decodes a little-endian encoded value from the buffer and stores it in `value`,
         * the number of bytes processed depends upon the type `T` of value passed
        ...
        /**
         * @brief Decodes a big-endian encoded value from the buffer and stores it in `value`, the
         * number of bytes processed depends upon the type `T` of value passed
         * @tparam T Type of value to be decoded, `sizeof(T)` bytes are read from the buffer
         * @param buffer Pointer to buffer which contains the encoded data
         * @param value Reference to a variable of type `T`, which will hold the decoded value
         * @return Number of bytes read from the buffer
         ...
         * @return Number of bytes written to the buffer
         * @note `buffer` should be of sufficient length to prevent overflow, i.e. it should be
         * atleast equal to the sum of sizes of the types passed
         */
        template <typename T, typename... Args>
        inline int encode_le(uint8_t *buffer, T value, Args... args)
        {
         ...
         */
        template <typename T, typename... Args>
        inline int encode_be(uint8_t *buffer, T value, Args... args)
        {
        ...
        /**
         * @brief Decodes multiple values in little-endian format and stores the decoded value in
         * the passed references
        ...
        /**
         * @brief Decodes multiple values in big-endian format and stores the decoded value in the
         * passed references
         * @tparam T Type of the first parameter to decode
         * @tparam Args Type of additional values to unpack from the buffer
         * @param buffer Pointer to the buffer where the encoded values are present
         * @param value The first value to decode
         * @param Args Additional values to unpack, each value will be decoded from the appropriate
         ...
         * @return The total number of bytes read from the buffer, or -1 if the length read exceeds
         * max_string_length
         * @note This function creates a new string, then moves it into s, so it is fine if s is
         * empty
         */
        template <endian endianness>
        inline int decode_length_prefixed(uint8_t *buffer, std::string &s, size_t max_string_length)
        {
            ...
            auto bytes_read
                = decode_length_prefixed<endianness>(buffer, str.data(), max_string_length);
            if (bytes_read == -1)
            {
                return -1;
            }
         ...
         * @return The total number of bytes read from the buffer, or -1 if the length read exceeds
         * max_length
         * @note This function creates a new vector, then moves it into v, so it is fine if v is
         * empty
         */
        template <endian endianness, typename T>
    ...
    /**
     * Functions in this namespace operate over streams - `istream` and `ostream`. They do not
     * require manual management of buffers
     */
    namespace stream
    {

        template <endian endianness, typename T>
        inline std::ostream &write(std::ostream &os, T value)
        {
        ...
        template <endian endianness, typename T> inline std::istream &
        read(std::istream &is, T &value, size_t max_elements = DEFAULT_MAX_NUMBER_OF_ELEMENTS)
        {
            std::vector<uint8_t> buffer;
            // Allocate a buffer of necessary size
            if constexpr (std::is_integral<T>::value || std::is_floating_point<T>::value)
                               ...
                               || std::is_same<T, std::vector<typename T::value_type>>::value)
            {
                ...
                if (sz > max_elements)
                {
                    throw std::runtime_error(
                        "Data contains more elements than max_elements, read failed");
                }
    ...
    namespace internal
    {
        ...
        /**
         * This function packs a double as an unsigned 64 bit integer, which can be written to a
         * buffer
         */
        template <unsigned bits, unsigned expbits> inline uint64_t pack754(long double f)
        {
            ...
            significand = static_cast<long long>(
                fnorm * (static_cast<float>(1LL << significandbits) + 0.5f));
        ...
        template <unsigned bits, unsigned expbits> inline long double unpack754(uint64_t i)
        {
            ...
            shift
                = static_cast<long long>(((i >> significandbits) & ((1LL << expbits) - 1)) - bias);
            while (shift > 0)
            {
                result *= 2.0;
                shift--;
